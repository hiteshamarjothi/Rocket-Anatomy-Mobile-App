...
function Falcon9Payload(props) {
    ...
    const progress = (icpsChecked + stageAdapterChecked) / 2 * 100;
    ...
    const storeData = async (value) => {
        try {
          ...
          await AsyncStorage.setItem('@storage_Key_UpperMid', jsonValue)
        } catch (e) {
          // saving error
        }
      }
    ...
    useEffect(() => {
        storeData({ icpsChecked, stageAdapterChecked });
    }, [icpsChecked, stageAdapterChecked]);
        ...
        <SafeAreaView style={styles.container}>
            ...
            <View style={styles.descriptionContainer}>
                ...
                <View style={styles.descriptionSection}>
                    <View style={styles.descriptionWrapper}>
                        <Animated.Text style={styles.payloadHeading} sharedTransitionTag="tag" >Upper-Mid</Animated.Text>
                        <Text style={styles.description}>The Upper-Mid section of the Artemis SLS Rocket contains the rockets and propulsion used for later periods in flight. It also houses the connections that connect first and second stage boosters. </Text>
                    </View>
                    <View style={styles.payloadImage}>
                        <Image style={styles.payloadImageSource} source={require('../../assets/UpperMid.png')}/>
                    </View>
                </View>
            ...
            <View style={styles.componentContainer}>
                <View style={styles.componentHeader}>
                    ...
                    <Text style={styles.componentHeaderSubText}>• Artemis Upper-Mid</Text>
                </View>
                <View style={styles.componentWrapper} >
                    <TouchableOpacity style={styles.fairingComponentItem} onPress={() => navigation.navigate('Artemis1ICPS')}>
                        <View style={styles.fairingWrapper}>
                            <View style={styles.fairingIcon}>
                                <Feather name="box" size={45} color="white" style={styles.icon}/>
                            </View>
                            <Text style={styles.header}>ICPS</Text>   
                        </View>

                        <View style={styles.negativeSpace}/>
                        <TouchableOpacity onPress={() => setIcpsChecked(!icpsChecked)} style={styles.fairingTickBox} activeOpactity={100}>
                            <View style={icpsChecked ? styles.checkedCircle : styles.fairingCircle}>
                                        {icpsChecked && <Feather name="check" size={24} color="white" />}
                            </View>
                        </TouchableOpacity>
                       
                            
                    </TouchableOpacity>
                    <TouchableOpacity style={styles.dragonComponentItem} onPress={() => navigation.navigate('Artemis1StageAdapter')}>
                        <View style={styles.dragonWrapper}>
                                <View style={styles.dragonIcon}>
                                    <Feather name="box" size={45} color="white" style={styles.icon}/>
                                </View>
                                <Text style={styles.header1} numberOfLines={2}>Stage Adapter</Text>   
                        </View>

                            
                            
                        <TouchableOpacity onPress={() => setStageAdapterChecked(!stageAdapterChecked)} style={styles.dragonTickBox}>
                        <View style={stageAdapterChecked ? styles.checkedCircle : styles.fairingCircle}>
                                        {stageAdapterChecked && <Feather name="check" size={24} color="white" />}
                            </View>
                        </TouchableOpacity>
